...
public class User extends Model<User> {
	...
	public User login(String userno,String password,String companyId){
		return this.findFirst("select u.*,per.*,p.name position_name,p.parent_id,de.name department_name from "+tableName+" u,"+Position.tableName+" p,"+Position.tableName+" de,"+Person.tableName+" per where per.id=u.id and de.id=p.parent_id and p.id=u.position_id and uname=? and password=? and u.company_id=? ",userno,password,companyId);
	}
	public User findById(String id,String company_id){
		return User.dao.findFirst("select  u.*,ps.realname,ps.email,ps.mobile,ps.sex,ps.head_pic,ps.telephone,ps.address,p.name as position_name,d.name as department_name from " + tableName + " u, "+Position.tableName+" p ,"+Position.tableName+" d ,"+Person.tableName+" ps where  u.id=ps.id and d.id=p.parent_id and u.position_id=p.id and u.id=? and u.company_id=?",id,company_id);
	}
	/**更新登录信息*/
